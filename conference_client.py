...
from conference_protocol import (
    ...
    encode_avatar_upload,
    encode_avatar_download,
    MESSAGE_DELIMITER,
)
...
class ConferenceClient:
    ...
    def upload_avatar(self, avatar_path: str) -> tuple[bool, str]:
        if not avatar_path or not self._username:
            return False, "invalid avatar or username"
        try:
            with open(avatar_path, "rb") as f:
                avatar_data = f.read()
        except Exception as e:
            return False, f"read avatar failed: {e}"
        msg = encode_avatar_upload(self._username, avatar_data)
        ok, resp_msg, _ = self._request_response(msg)
        return ok, resp_msg

    def download_avatar(self, target_user: str = "") -> tuple[bool, str, bytes]:
        target = target_user.strip() or self._username
        msg = encode_avatar_download(self._username, target)
        ok, resp_msg, data = self._request_response(msg)
        if ok and data.get("avatar_data"):
            try:
                avatar_data = base64.b64decode(data["avatar_data"])
                return True, resp_msg, avatar_data
            except Exception:
                return False, "decode failed", b""
        return False, resp_msg, b""
