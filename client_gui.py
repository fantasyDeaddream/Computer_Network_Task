...
class LoginFrame(ttk.Frame):
    ...
    def _handle_result(self, client, success, msg):
        ...
        if success:
            username = self.username_var.get().strip()
            self._sync_avatar_from_server(client, username)
            self._on_login_success(client, username)
        else:
            messagebox.showerror("登录失败", msg)

    def _sync_avatar_from_server(self, client: ConferenceClient, username: str):
        def do_sync():
            ok, msg, avatar_data = client.download_avatar(username)
            if ok and avatar_data:
                avatars_dir = Path(__file__).resolve().parent / "data" / "avatars"
                avatars_dir.mkdir(parents=True, exist_ok=True)
                dest_path = avatars_dir / f"{username}.png"
                try:
                    with open(dest_path, "wb") as f:
                        f.write(avatar_data)
                    print(f"[Client] Avatar synced from server for {username}")
                except Exception as e:
                    print(f"[Client] Failed to save synced avatar: {e}")
        threading.Thread(target=do_sync, daemon=True).start()


# ============================================================
# ContactFrame - 联系人界面（显示在线状态，撤销双击呼叫功能）
# 新增：显示联系人在线/离线状态
# ============================================================
...
class AvatarSelectorFrame(ttk.Frame):
    ...
    def __init__(self, parent, username: str, client: ConferenceClient = None):
        super().__init__(parent)
        self._username = username
        self._client = client
    ...
    def _on_confirm(self):
        ...
        try:
            ...
            if self._client:
                def upload():
                    ok, msg = self._client.upload_avatar(str(dest_path))
                    self.after(0, lambda: self._show_upload_result(ok, msg))
    ...
    def _show_upload_result(self, ok: bool, msg: str):
        if ok:
            messagebox.showinfo("成功", "头像已同步到服务器！")
        else:
            messagebox.showwarning("同步失败", f"服务器同步失败: {msg}")
...
class ChatRoomFrame(ttk.Frame):
    ...
    def _sync_avatar_for_user(self, username: str) -> None:
        if username == self._client.username:
            return
        avatar_path = self._AVATARS_DIR / f"{username}.png"
        if avatar_path.is_file():
            return
        def do_sync():
            ok, msg, avatar_data = self._client.download_avatar(username)
            if ok and avatar_data:
                self._AVATARS_DIR.mkdir(parents=True, exist_ok=True)
                try:
                    with open(avatar_path, "wb") as f:
                        f.write(avatar_data)
                    self.after(0, lambda: self._on_avatar_synced(username))
                except Exception:
                    pass
        threading.Thread(target=do_sync, daemon=True).start()

    def _on_avatar_synced(self, username: str) -> None:
        for pos, uname in list(self._members.items()):
            if uname == username:
                canvas, label = self._avatar_widgets[pos]
                canvas.delete("all")
                avatar_img = self._load_avatar(username)
                if avatar_img is not None:
                    self._avatar_images[pos] = avatar_img
                    canvas.create_image(
                        self.AVATAR_SIZE // 2,
                        self.AVATAR_SIZE // 2,
                        image=avatar_img,
    ...
    def _refresh_members(self, members, positions):
        ...
        for m in members:
            ...
            if avatar_img is not None:
                ...
                self._draw_default_avatar(canvas)
                self._sync_avatar_for_user(uname)
...
class ClientGUI(ttk.Window):
    ...
    def _build_main_ui(self):
        ...
        self._avatar_selector_frame = AvatarSelectorFrame(
            self._notebook, self._username, self._client
        )
