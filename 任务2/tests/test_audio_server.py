...
class TestAudioServer:
    ...
    def test_server_broadcast_excludes_sender(self):
        ...
        assert sent_message["filename"] == "test.wav"
...
class TestAudioServerIntegration:
    ...
    def test_private_text_message_delivery(self):
        """测试文本单播只发送给指定接收方"""
        server = AudioServer()
        sender_conn = Mock()
        sender_conn.sendall = Mock()
        target_conn = Mock()
        target_conn.sendall = Mock()
        other_conn = Mock()
        other_conn.sendall = Mock()

        server._AudioServer__connections = [None, sender_conn, target_conn, other_conn]
        server._AudioServer__nicknames = ["System", "Alice", "Bob", "Carol"]
        ...
        server._AudioServer__handle_message(1, message)

        target_conn.sendall.assert_called()
        other_conn.sendall.assert_not_called()
