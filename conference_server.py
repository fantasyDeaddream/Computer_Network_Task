"""
SEU Meeting 会议服务器，支持按客户端自适应下行音频传输。
"""
...
class ConferenceServer:
    def __init__(self, host: str = "0.0.0.0", port: int = DEFAULT_PORT) -> None:
        ...
        self._avatars_dir = os.path.join(os.path.dirname(__file__), "data", "avatars")
    ...
    def _dispatch(self, cid, message_type, payload, conn) -> None:
        handlers = {
            ...
            "avatar_upload": lambda: self._handle_avatar_upload(cid, payload),
            "avatar_download": lambda: self._handle_avatar_download(cid, payload),
        }
    ...
    def _handle_avatar_upload(self, cid, payload) -> None:
        with self._lock:
            info = self._clients.get(cid)
            if not info or not info.username:
                self._send_by_cid(cid, encode_response(False, "not logged in"))
            ...
            username = info.username
        try:
            avatar_data = base64.b64decode(payload.get("data", ""))
        except Exception:
            self._send_by_cid(cid, encode_response(False, "invalid avatar data"))
            return
        if not avatar_data:
            self._send_by_cid(cid, encode_response(False, "empty avatar data"))
            return
        avatar_path = os.path.join(self._avatars_dir, f"{username}.png")
        try:
            with open(avatar_path, "wb") as f:
                f.write(avatar_data)
            self._send_by_cid(cid, encode_response(True, "avatar uploaded"))
            print(f"[Server] Avatar saved for {username}")
        except Exception as e:
            self._send_by_cid(cid, encode_response(False, f"save failed: {e}"))

    def _handle_avatar_download(self, cid, payload) -> None:
        with self._lock:
            info = self._clients.get(cid)
            if not info or not info.username:
                self._send_by_cid(cid, encode_response(False, "not logged in"))
                return
        target_user = payload.get("target_user", "") or info.username
        avatar_path = os.path.join(self._avatars_dir, f"{target_user}.png")
        if not os.path.exists(avatar_path):
            self._send_by_cid(cid, encode_response(False, "avatar not found"))
            return
        try:
            ...
            self._send_by_cid(
                cid, encode_response(True, "avatar ok", {"avatar_data": avatar_b64})
            )
        except Exception as e:
            self._send_by_cid(cid, encode_response(False, f"read failed: {e}"))
