...
from tk_utils import redirect_stdout_to_queue, safe_int
...
class ServerGUI(tk.Tk):
    def __init__(self) -> None:
        super().__init__()
        self.title("任务3 - 音频通信服务器（Tkinter）")
    ...
    def _build_ui(self) -> None:
        ...
        self.port_var = tk.StringVar(value="8880")
        ttk.Entry(cfg, textvariable=self.port_var, width=10).grid(row=0, column=1, padx=(8, 16), sticky="w")
        ...
        hint = "说明：本窗口启动的是任务3实现的“实时音频流”服务器，与任务2的文件式服务器协议不同。"
    ...
    def _on_start(self) -> None:
        ...
        port = safe_int(self.port_var.get(), 8880)

        def spawn() -> None:
            try:
                self._server = StreamServer(port=port)
